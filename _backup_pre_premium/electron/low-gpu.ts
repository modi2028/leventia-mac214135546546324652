...
// Every FastFlag key this module may write — used when stripping our changes on restore.
const ALL_MANAGED_KEYS = [...Object.keys(LOW_GPU_FLAGS), FPS_FLAG]
...
export function buildFlags(lowGpu: boolean, fpsCap: number): Record<string, string> {
  const out: Record<string, string> = {}
  if (lowGpu) for (const [k, v] of Object.entries(LOW_GPU_FLAGS)) out[k] = String(v)
  const fps = fpsCap > 0 ? Math.round(fpsCap) : (lowGpu ? 30 : 0)
...
export function applyPerformance(opts: { lowGpu: boolean; fpsCap: number }, playerExe?: string): boolean {
  const flags = buildFlags(opts.lowGpu, opts.fpsCap)
