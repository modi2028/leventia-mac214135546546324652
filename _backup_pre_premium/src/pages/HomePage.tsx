...
export function HomePage({ license, onNavigate }: Props) {
  ...
  const tiles = [
    { key: 'accounts', label: 'Accounts', value: total,   tint: '167,139,250', sub: noCookie > 0 ? `${noCookie} without cookie` : 'all have cookies' },
    { key: 'healthy',  label: 'Healthy',  value: healthy, tint: '74,222,128',  sub: total ? `${Math.round((healthy / total) * 100)}% of fleet` : 'cookies valid' },
    { key: 'expired',  label: 'Expired',  value: expired, tint: '248,113,113', sub: expired > 0 ? 'need a refresh' : 'all good' },
    { key: 'running',  label: 'Running',  value: runCount, tint: '103,232,249', sub: runCount === 1 ? 'alt launched' : 'alts launched' },
  ]
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', overflowX: 'hidden', paddingRight: 6, paddingBottom: 8 }}>
      {/* Greeting + live clock */}
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 18 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: '#fff', margin: 0, letterSpacing: '-0.01em' }}>
          ...
          <p style={{ fontSize: 12, color: 'oklch(0.74 0.035 283)', marginTop: 5 }}>
            {runCount > 0 ? <>You have <b style={{ color: '#67e8f9' }}>{runCount}</b> alt{runCount === 1 ? '' : 's'} running right now.</> : "Here's your fleet at a glance."}
          </p>
        </div>
        <div className="glass" style={{ padding: '9px 14px', borderRadius: 12, textAlign: 'right', flexShrink: 0 }}>
          <div className="glass-aurora" />
          <p style={{ fontSize: 20, fontWeight: 800, color: '#fff', fontFamily: '"JetBrains Mono", monospace', lineHeight: 1, margin: 0 }}>{clock}</p>
          <p style={{ fontSize: 9.5, color: 'oklch(0.68 0.035 281)', marginTop: 4 }}>{today}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 12 }}>
        {tiles.map(t => (
          <div key={t.key} className="glass lift" style={card}>
            <div className="glass-aurora" />
            ...
            <p style={{ fontSize: 34, fontWeight: 800, color: `rgb(${t.tint})`, fontFamily: '"JetBrains Mono", monospace', lineHeight: 1, margin: '10px 0 4px' }}>{t.value}</p>
            <p style={{ fontSize: 9.5, color: 'oklch(0.66 0.03 280)', margin: 0 }}>{t.sub}</p>
          </div>
        ))}
      </div>

      {/* Fleet health bar */}
      <div className="glass" style={{ ...card, marginBottom: 12 }}>
        <div className="glass-aurora" />
      ...
      {/* Recent activity */}
      <div className="glass" style={{ ...card, marginTop: 12 }}>
        <div className="glass-aurora" />
