...
import type { AppSettings, AntiAfkStatus, RobloxAccount } from '../types'
...
export function PremiumPage() {
  ...
  const applyPerf = async (next: AppSettings) => {
    if (next.lowGpuEnabled || (next.fpsCap ?? 0) > 0) await window.electron.lowGpu.apply().catch(() => {})
    else                                              await window.electron.lowGpu.restore().catch(() => {})
  }
      ...
      <div style={{ maxWidth: tab === 'servermap' ? 920 : 560 }}>
        ...
        <div className="glass" style={{ padding: 18, marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
            {([['Unlimited', 0], ['30', 30], ['60', 60], ['120', 120], ['144', 144], ['240', 240]] as const).map(([label, v]) => {
        ...
        <div style={{ marginTop: 16, padding: '12px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <p style={{ fontSize: 11, color: 'oklch(0.8 0.035 283)', lineHeight: 1.7, margin: 0 }}>
            ...
            to <strong>Unlimited</strong> and Low GPU <strong>off</strong> restores your original graphics.
          </p>
        </div>
        </>)}
