...
// Default channel for infraction logs when none is set via /setup.
const INFRACTION_CHANNEL_ID = '1484647129606783127';

// Posts the panel to the configured (or default) infraction-log channel.
// Returns { ch, error } so callers can tell the user what actually happened
// instead of silently swallowing a failed send and falsely claiming success.
async function logPanel(g, payload) {
    const ch = (await resolveChannel(g, cfg(g.id).infractionLogChannel))
        || (await resolveChannel(g, INFRACTION_CHANNEL_ID));
...
// Build the trailing note for the reply based on the send outcome.
