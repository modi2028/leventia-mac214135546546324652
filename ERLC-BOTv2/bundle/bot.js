...
function isSupport(member, cfg15) {
  if (isStaff(member)) return true;
  const roles = csv(cfg15?.supportRoles);
...
var SESSION_CHANNEL_ID = "1503032169285816341";
async function targetChannel(g, fallback) {
  return await resolveChannel(g, cfg(g.id).sessionChannel) || await resolveChannel(g, SESSION_CHANNEL_ID) || fallback;
}
var PANELS = {
...
var INFRACTION_CHANNEL_ID = "1484647129606783127";
async function logPanel(g, payload) {
  const ch = await resolveChannel(g, cfg2(g.id).infractionLogChannel) || await resolveChannel(g, INFRACTION_CHANNEL_ID);
...
function logNote({ ch, error }) {
  if (error === "no-channel") return " \u26A0\uFE0F No infraction-log channel set \u2014 use `/setup set-channel infraction-log`.";
...
async function handleSlash4(interaction) {
  ...
  if (sub === "add") {
    ...
    const res = await logPanel(g, infractionPanel(g, rec));
    user2.send(`You received an infraction in **${g.name}** \u2014 **${rec.type}**: ${rec.reason}`).catch(() => {
    });
    return interaction.reply({ embeds: [okEmbed(`\u2705 Infraction **#${rec.id}** added.${logNote(res)}`)], ephemeral: true });
  }
  if (sub === "remove") {
...
async function handlePrefix4(cmd, message, args, rest) {
  ...
  const res = await logPanel(message.guild, infractionPanel(message.guild, rec));
  user.send(`You received an infraction in **${message.guild.name}** \u2014 **${type}**: ${reason}`).catch(() => {
  });
  return message.reply({ embeds: [okEmbed(`\u2705 Infraction **#${rec.id}** added.${logNote(res)}`)] }).catch(() => {
  });
}
...
var PROMO_CHANNEL_ID = "1484647040569970959";
async function promoChannel(g) {
  return await resolveChannel(g, cfg3(g.id).promoChannel) || await resolveChannel(g, PROMO_CHANNEL_ID);
}
...
async function handleSlash5(interaction) {
  ...
  if (sub === "add") {
    ...
    const res = await postPromo(g, promotionPanel(g, rec, user2.tag));
    user2.send(`\u{1F389} You were promoted to **${rec.toRank}** in **${g.name}**!`).catch(() => {
    });
    return interaction.reply({ embeds: [okEmbed(`\u2705 Promotion logged${logNote2(res)}`)], ephemeral: true });
  }
  const user = interaction.options.getUser("user");
...
async function handlePrefix5(cmd, message, args, rest) {
  ...
  return message.reply({ embeds: [okEmbed(`\u2705 Promotion logged${logNote2(res)}`)] }).catch(() => {
  });
}
...
var TICKET_TYPES_VERSION = 3;
function types(gid) {
  const c = cfg4(gid);
  if (c.ticketTypesSeedVersion === void 0) {
...
function canHandleTicket(member, g, t) {
  if (isSupport(member, cfg4(g.id))) return true;
...
async function handleSlash6(interaction) {
  ...
  if (name === "ticket") {
    ...
    if (sub === "reset-types") {
      ...
      cfg4(g.id).ticketTypesSeedVersion = TICKET_TYPES_VERSION;
      persist();
    ...
    if (sub === "type-list") {
      const arr = types(g.id);
      const lines = arr.map((t2) => {
        const pr = ticketPingRoleId(g, t2);
        const viewers = viewerRoleIds(g, t2).filter((rid) => g.roles.cache.has(rid));
        const access = viewers.length ? viewers.map((rid) => `<@&${rid}>`).join(" ") : "\u2014";
        const head = `${t2.emoji ? t2.emoji + " " : ""}**${t2.label}** \`${t2.id}\`${t2.category ? ` \u2192 <#${t2.category}>` : ""}`;
        return `${head}
> Pings: ${pr ? `<@&${pr}>` : "\u2014"}
> Access: ${access}`;
      });
...
async function handleComponent3(interaction) {
  ...
  if (action === "forward") {
    ...
    return interaction.channel.send({ content: fwdPing ? `<@&${fwdPing}>` : void 0, embeds: [infoEmbed("Ticket Forwarded", `This ticket was forwarded to **${target.label}** by ${interaction.user}.`)], allowedMentions: { roles: fwdPing ? [fwdPing] : [] } }).catch(() => {
    });
  }
}
...
var slash15 = [];
var owns15 = [];
var GIVEAWAY_CHANNEL_ID = "1503029371307557037";
var ordinal = (n) => {
...
function welcomePayload(member) {
  ...
  const row = new ActionRowBuilder8().addComponents(
    // Join-position chip — disabled so it's a label, not a real button.
    new ButtonBuilder6().setCustomId("welcome:count").setStyle(ButtonStyle6.Secondary).setLabel(ordinal(count)).setDisabled(true)
  );
  if (/^https?:\/\//i.test(b.joinUrl || "")) {
    row.addComponents(new ButtonBuilder6().setStyle(ButtonStyle6.Link).setLabel("Dashboard").setURL(b.joinUrl));
  }
  return {
    content: `\u{1F44B} Thanks for joining **${emoji}${name}**, <@${member.id}>! We are so glad to have you apart of this community. Make sure to join our **250k R$** giveaway in <#${GIVEAWAY_CHANNEL_ID}> and possibly win this awesome prize!`,
    components: [row],
    allowedMentions: { users: [member.id] }
    // ping just the new member
  };
}
function init3(ctx) {
...
var reviewRow = (done = false) => new ActionRowBuilder9().addComponents(
  new ButtonBuilder7().setCustomId("media:accept").setLabel("Accept").setStyle(ButtonStyle7.Success).setDisabled(done),
  new ButtonBuilder7().setCustomId("media:deny").setLabel("Deny").setStyle(ButtonStyle7.Danger).setDisabled(done)
);
...
import { SlashCommandBuilder as SlashCommandBuilder18, ButtonBuilder as ButtonBuilder8, ButtonStyle as ButtonStyle8 } from "discord.js";
...
function verifyPanel(gid) {
  ...
  return panel({
    ...
    buttons: [new ButtonBuilder8().setCustomId("verify:go").setLabel("Verify").setStyle(ButtonStyle8.Success)]
  });
}
var slash19 = [
...
import { SlashCommandBuilder as SlashCommandBuilder19, ButtonBuilder as ButtonBuilder9, ButtonStyle as ButtonStyle9 } from "discord.js";
...
async function buildStatusPanel(gid) {
  ...
  fields.push({ name: "Last updated", value: ts(now()) });
  const url = b.joinUrl || (s?.JoinKey ? `https://policeroleplay.community/join?code=${s.JoinKey}` : "");
  const btn = /^https?:\/\//i.test(url) ? [new ButtonBuilder9().setStyle(ButtonStyle9.Link).setLabel("Join Server").setURL(url)] : void 0;
  return panel({
    guildId: gid,
    kind: "status",
...
import { SlashCommandBuilder as SlashCommandBuilder20, ButtonBuilder as ButtonBuilder10, ButtonStyle as ButtonStyle10, EmbedBuilder as EmbedBuilder4, version as djsVersion } from "discord.js";
var clientRef4 = null;
var reminderTimers = /* @__PURE__ */ new Map();
var polls = (gid) => guild(gid).polls ||= {};
function pollButtons(rec) {
  return rec.options.map((o, i) => new ButtonBuilder10().setCustomId(`poll:${i}`).setLabel(`${o.label} (${o.voters.length})`).setStyle(ButtonStyle10.Secondary));
}
...
import {
  ...
  ButtonBuilder as ButtonBuilder11,
  ButtonStyle as ButtonStyle11,
  ActionRowBuilder as ActionRowBuilder10,
...
function appPanel(gid, app) {
  return panel({
    ...
    footer: branding(gid).name || void 0,
    buttons: [new ButtonBuilder11().setCustomId(`app:${app.id}`).setLabel(app.buttonLabel || `Apply for ${app.name}`).setStyle(ButtonStyle11.Primary).setDisabled(app.open === false)]
  });
}
var reviewRow2 = (disabled = false) => new ActionRowBuilder10().addComponents(
  new ButtonBuilder11().setCustomId("appreview:accept").setLabel("Accept").setStyle(ButtonStyle11.Success).setDisabled(disabled),
  new ButtonBuilder11().setCustomId("appreview:deny").setLabel("Deny").setStyle(ButtonStyle11.Danger).setDisabled(disabled)
);
...
// features/information.js
var information_exports = {};
__export(information_exports, {
  handleSlash: () => handleSlash22,
  owns: () => owns23,
  slash: () => slash23
});
import {
  SlashCommandBuilder as SlashCommandBuilder22,
  ContainerBuilder as ContainerBuilder2,
  MediaGalleryBuilder as MediaGalleryBuilder2,
  MediaGalleryItemBuilder as MediaGalleryItemBuilder2,
  TextDisplayBuilder as TextDisplayBuilder2,
  SeparatorBuilder as SeparatorBuilder2,
  MessageFlags as MessageFlags2
} from "discord.js";
var cfg14 = (gid) => guild(gid).config;
var INFO_BANNER = "https://cdn.discordapp.com/attachments/1471572008356675585/1514735994451001417/Info.png?ex=6a3067d3&is=6a2f1653&hm=f8aa0049c917eb0b73dadfec56ba759ef6d9aa01f5897dec79571dbd8527c8cc&";
var FOOTER_BANNER = "https://cdn.discordapp.com/attachments/1471572008356675585/1515321820264529940/Footer_banner.png?ex=6a308f2b&is=6a2f3dab&hm=1b87f2277579ed5e466809c75c4eda608657c214d3ba4a932a7b6b62c37a63ad&";
var INFO_TEXT = "Welcome to <:Florida:1485730645643886663> **Florida State Roleplay!** Florida State Roleplay is a realistic and community-focused roleplay server inspired by the diverse landscapes, cities, and culture of the State of Florida. Our goal is to provide an immersive and enjoyable experience for all players, whether you prefer law enforcement, fire and rescue, transportation services, or civilian roleplay.\n\n**Important Links**\n- [Server Regulations](https://discord.com/channels/1484625566370889740/1503031179782258818)\n- [Applications](https://discord.com/channels/1484625566370889740/1484634773967208529)\n- [Server Support](https://discord.com/channels/1484625566370889740/1484631976064585879)\n- [Community Group](https://www.roblox.com/communities/651302991/Florida-State-Roleplay-NEW-ERLC#!/about)\n- [Whitelisted Group](https://www.roblox.com/communities/893298210/Florida-State-Roleplay-Strict-Whitelisted#!/about)";
function informationPanel() {
  const c = new ContainerBuilder2();
  c.addMediaGalleryComponents(
    new MediaGalleryBuilder2().addItems(new MediaGalleryItemBuilder2().setURL(INFO_BANNER))
  );
  c.addTextDisplayComponents(new TextDisplayBuilder2().setContent(INFO_TEXT));
  c.addSeparatorComponents(new SeparatorBuilder2());
  c.addMediaGalleryComponents(
    new MediaGalleryBuilder2().addItems(new MediaGalleryItemBuilder2().setURL(FOOTER_BANNER))
  );
  return { components: [c], flags: MessageFlags2.IsComponentsV2 };
}
var slash23 = [
  new SlashCommandBuilder22().setName("information").setDescription("Post the server information panel").addChannelOption((o) => o.setName("channel").setDescription("Channel to post in (default: here)"))
];
var owns23 = ["information"];
async function handleSlash22(interaction) {
  if (!isStaff(interaction.member)) return interaction.reply({ embeds: [errEmbed("Staff only.")], ephemeral: true });
  const ch = interaction.options.getChannel("channel") || await resolveChannel(interaction.guild, cfg14(interaction.guild.id).informationChannel) || interaction.channel;
  try {
...
var slash24 = [
  new SlashCommandBuilder23().setName("regulations").setDescription("Post the regulations hub (with the rules dropdown)").addChannelOption((o) => o.setName("channel").setDescription("Channel to post in (default: here)")),
  new SlashCommandBuilder23().setName("discord-rules").setDescription("Post the Discord regulations").addChannelOption((o) => o.setName("channel").setDescription("Channel to post in (default: here)")),
  new SlashCommandBuilder23().setName("roblox-rules").setDescription("Post the in-game (Roblox) regulations").addChannelOption((o) => o.setName("channel").setDescription("Channel to post in (default: here)"))
];
var owns24 = ["regulations", "discord-rules", "roblox-rules"];
var componentNs11 = ["regrules"];
...
async function handleSlash23(interaction) {
  const name = interaction.commandName;
...
async function handleComponent11(interaction) {
  if (!interaction.isStringSelectMenu()) return;
  const text = interaction.values[0] === "discord" ? DISCORD_RULES_TEXT : ROBLOX_RULES_TEXT;
  const p = regPanel(text);
  return interaction.reply({ ...p, flags: V22 | MessageFlags3.Ephemeral }).catch(() => {
  });
}

// bot.js
var MODULES = [erlc_exports, sessions_exports, giveaways_exports, infractions_exports, promotions_exports, tickets_exports, afk_exports, reviews_exports, training_exports, suggestions_exports, roleplay_exports, misc_exports, setup_exports, dashboard_exports, welcome_exports, media_exports, moderation_exports, globalban_exports, verify_exports, status_exports, utility_exports, applications_exports, information_exports, regulations_exports];
