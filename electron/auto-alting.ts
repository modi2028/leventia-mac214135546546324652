...
import { deployErlc, removeRunning, getRunningIds, getRunningPidMap } from './ipc/roblox.js'
import { startAutoJail, stopAutoJail, syncJail } from './auto-jail.js'
import { trimPids, setLoadingPids } from './resource-trim.js'
...
// Adaptive poll delay (ms). We poll at the user's interval normally, but speed up
// to ~10s when the server is near capacity so we pull alts out FAST the moment real
// players need the slots (floored at 8s so we never hammer PRC into a rate limit).
let dynamicDelayMs = 30_000

// account IDs we have deployed (and believe are in / joining the server)
const deployedIds = new Set<string>()

// Per-account join lifecycle (for aggressive, per-account Resource Trim):
//   launchedAt   → when we spawned it (so we can report its individual load time)
//   confirmedIds → PRC has confirmed it inside the server
//   minimizedIds → we've already minimized/trimmed it (don't repeat)
// Each alt loads on its own timeline, so we minimize each ONE the moment it's
// confirmed in-server — not on a blunt global timer — and we keep alts that are
// still loading OUT of the trim sweep so minimizing can't slow their join.
const launchedAt   = new Map<string, number>()
const confirmedIds = new Set<string>()
const minimizedIds = new Set<string>()

function clearJoinState(id: string): void {
  launchedAt.delete(id); confirmedIds.delete(id); minimizedIds.delete(id)
}
...
async function deployWave(count: number): Promise<number> {
  ...
  for (let i = 0; i < toDeploy.length; i++) {
    ...
    if (res.success) { done++; launchedAt.set(acc.id, Date.now()); log(`✓ Launched ${acc.username} (${done}/${toDeploy.length})`) }
    else { deployedIds.delete(acc.id); clearJoinState(acc.id); log(`✗ Failed ${acc.username}: ${res.error ?? 'error'}`) }
...
async function tick(): Promise<void> {
  ...
  try {
    ...
    const s = getSettings()
    const perfOn = !!(s.autoTrim || s.pixelReduceEnabled || s.ramLimitEnabled)
    let pending = 0
    const inServerAlts: string[] = []        // usernames of OUR alts PRC reports in-server (for Auto Jail)
    const newlyConfirmedPids: number[] = []  // alts that JUST confirmed in-server → minimize now
    const loadingPids: number[] = []         // alts still loading → keep out of the trim sweep
    for (const id of [...deployedIds]) {
      const acc = accounts.find(a => a.id === id)
      const inServer = acc ? srv.names.includes(acc.username.toLowerCase()) : false
      if (inServer) {
        if (acc) inServerAlts.push(acc.username)
        // First time we see this alt in-server: it's confirmed. Report its own load
        // time (each alt joins on its own timeline) and minimize its tab once.
        if (!confirmedIds.has(id)) {
          confirmedIds.add(id)
          const ms = launchedAt.get(id)
          const secs = ms ? Math.round((Date.now() - ms) / 1000) : null
          if (acc) log(`✅ ${acc.username} confirmed in-server${secs != null ? ` (loaded in ${secs}s)` : ''}`)
        }
        const pid = pidMap[id]
        if (perfOn && pid && !minimizedIds.has(id)) { minimizedIds.add(id); newlyConfirmedPids.push(pid) }
        continue
      }
      if (running.has(id)) {
        pending++                             // launched, still loading in
        // Not yet confirmed → keep its window out of the trim sweep so minimizing
        // doesn't throttle the client while it's still joining.
        const pid = pidMap[id]
        if (pid && !confirmedIds.has(id)) loadingPids.push(pid)
      } else {
        deployedIds.delete(id)                // process gone — free it up
        clearJoinState(id)
        failCooldown.set(id, Date.now() + FAIL_COOLDOWN_MS)   // …but cool it down so we don't spam-redeploy a client that won't stay
      }
    }

    // Drive aggressive, per-account Resource Trim: minimize the alts that just
    // confirmed in-server, and keep the still-loading ones excluded from the sweep.
    setLoadingPids(loadingPids)
    if (newlyConfirmedPids.length) void trimPids(newlyConfirmedPids)

    // Auto Jail: queue our in-server alts for the in-game jail via :jail <user>.
    // No-ops unless the autoJail setting is on; only queues alts not already jailed
    // or queued. A background worker drains the queue at the user's paced delay with
    // rate-limit backoff (see auto-jail.ts), so this never bursts the PRC API.
    if (inServerAlts.length) {
      const queued = syncJail(cfg.serverKey, inServerAlts)
      if (queued.length) log(`🔒 Queued ${queued.length} alt${queued.length > 1 ? 's' : ''} for jailing…`)
    }
    ...
    // Adaptive pacing: when the server is near capacity, poll fast so we pull alts
    // the instant real players need slots; otherwise use the user's interval.
    const near = srv.players >= Math.max(0, cfg.removeAt - 3)
    dynamicDelayMs = Math.max(8, near ? Math.min(cfg.interval, 10) : cfg.interval) * 1000
  } finally {
    busy = false
  }
}
...
export function startAutoAlt(config: AutoAltConfig): { ok: boolean; error?: string } {
  ...
  wasFull = false
  launchedAt.clear(); confirmedIds.clear(); minimizedIds.clear()
  dynamicDelayMs = Math.max(8, cfg.interval) * 1000
  status.running = true
  log('Automation started.')
  // Run the Auto Jail loop alongside automation. It self-gates on the `autoJail`
  // setting, so it costs nothing when the user hasn't enabled jailing.
  startAutoJail()
  // Self-scheduling loop (instead of a fixed interval) so the poll cadence can adapt
  // to server load — see dynamicDelayMs. Each run waits for the previous tick.
  const loop = async () => {
    try { await tick() } catch (e) { log('Tick error: ' + ((e as Error)?.message ?? e)) }
    if (status.running) timer = setTimeout(() => void loop(), dynamicDelayMs)
  }
  void loop()
  return { ok: true }
}
...
export function stopAutoAlt(): void {
  ...
  stopAutoJail()
  launchedAt.clear(); confirmedIds.clear(); minimizedIds.clear()
  setLoadingPids([])   // release the trim sweep's loading exclusion
