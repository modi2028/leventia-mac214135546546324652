...
import { getAutoJailStatus } from '../auto-jail.js'
...
export function registerAutoAltHandlers(): void {
  ...
  // Auto Jail — live status (loop running? setting on? how many alts jailed?). The
  // toggle itself is persisted in settings via store:save-settings.
  ipcMain.handle('autoalt:jail-status', () => getAutoJailStatus())
}
