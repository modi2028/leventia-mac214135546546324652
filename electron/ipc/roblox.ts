...
import { applyPerformance, anyPerfFlag, clientSettingsForExe } from '../low-gpu.js'
...
// Find the Roblox client using platform abstraction
function findRobloxPlayer(): { type: RobloxLauncherType; path: string } | null {
  return launcher.findPlayer()
}
...
async function wipeRobloxCookies(): Promise<void> {
  const cookiesPath = launcher.getCookiePath()
...
async function doJoin(
  ...
  try {
    ...
    try {
      ...
      const perfOpts = { lowGpu: s.lowGpuEnabled, fpsCap, disableTextures: s.disableTextures, minimalLighting: s.minimalLighting, skipSplash: s.skipSplash }
      if (anyPerfFlag(perfOpts)) {
        ...
        const ok = applyPerformance(perfOpts, robloxPath)
        launchLog(ok
          ? `applied performance FastFlags (lowGpu=${s.lowGpuEnabled}, fps=${fpsCap > 0 ? fpsCap : (s.lowGpuEnabled ? 30 : '∞')}, textures=${!s.disableTextures}, lighting=${s.minimalLighting ? 'min' : 'default'}) → ${clientSettingsForExe(robloxPath)}`
    ...
    try { const ms = getSettings(); if (ms.antiAfkMute || ms.muteMusic) setTimeout(() => setMuteRoblox(true), 9000) } catch {}
...
export function removeRunning(accountId: string): boolean {
  ...
  const pid = proc.pid
  // proc.kill() sends SIGTERM, which GUI apps may ignore
  // — so a "removed" alt could stay in the game. Force-kill the whole process tree by PID
  try {
    if (pid) launcher.killProcess(pid)
    else proc.kill()
