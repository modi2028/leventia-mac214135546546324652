...
// Public: the current announcement everyone shows. Empty when none set.
export async function supabaseGetAnnouncement(): Promise<{ text?: string; level?: string; at?: string }> {
  const r = await rpc<{ text?: string; level?: string; at?: string }>('rpc_get_announcement', {})
  if (!r.ok || !r.data) return {}
  return { text: r.data.text || undefined, level: r.data.level || undefined, at: r.data.at || undefined }
}

// Owner-only: set (or clear, via empty text) the broadcast announcement.
export async function supabaseSetAnnouncement(ownerSecret: string, text: string, level: string): Promise<{ ok: boolean; text?: string; error?: string }> {
  const r = await rpc<{ ok: boolean; text?: string; error?: string }>('rpc_set_announcement', { p_owner_secret: ownerSecret, p_text: text, p_level: level })
  if (!r.ok) return { ok: false, error: r.error ?? 'Could not reach the server.' }
  return r.data ?? { ok: false, error: 'No response.' }
}
