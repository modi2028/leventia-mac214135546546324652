...
// ── Send an in-game command via the PRC API ───────────────────────────────────
// POST /server/command runs a server-level moderation command in ER:LC (e.g.
// ":jail SomeUser"). Requires the server key; returns 200 with an empty body on
// success. The command MUST include the in-game ":" prefix.
export async function sendErlcCommand(serverKey: string, command: string): Promise<{ ok: boolean; error?: string; retriable?: boolean; rateLimited?: boolean }> {
  const key = serverKey.trim()
  if (!key) return { ok: false, error: 'No server key provided.', retriable: false }
  try {
    const res = await fetch(`${BASE}/server/command`, {
      method: 'POST',
      headers: { 'server-key': key, 'Accept': 'application/json', 'Content-Type': 'application/json' },
      body: JSON.stringify({ command }),
    })
    if (res.ok) return { ok: true }
    // 429 = PRC rate limit → wait + retry. 5xx = transient PRC error → retry too.
    if (res.status === 429) return { ok: false, error: 'Rate limited by PRC.', retriable: true, rateLimited: true }
    // 401/403 = bad key / no command perms → permanent, don't keep retrying.
    if (res.status === 401 || res.status === 403) return { ok: false, error: 'Invalid server key or command not permitted.', retriable: false }
    let detail = ''
    try { const d = await res.json() as { message?: string }; detail = d?.message ? ` — ${d.message}` : '' } catch {}
    return { ok: false, error: `PRC ${res.status}${detail}`, retriable: res.status >= 500 }
  } catch (err) {
    return { ok: false, error: err instanceof Error ? err.message : 'Network error.', retriable: true }
  }
}
