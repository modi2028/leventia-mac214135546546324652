...
export async function handleSlash(interaction) {
    ...
    if (sub === 'add') {
        ...
        const ch = await resolveChannel(g, cfg(g.id).promoChannel);
        if (ch) await ch.send(promotionPanel(g, rec, user.tag)).catch(() => {});
...
export async function handlePrefix(cmd, message, args, rest) {
    ...
    const ch = await resolveChannel(message.guild, cfg(message.guild.id).promoChannel);
