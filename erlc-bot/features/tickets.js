...
// Default set. Each gets the brand emoji (if configured) and its own auto-created
// category named after the label, so tickets sort neatly in the channel list.
...
// Roles allowed to SEE a ticket of this type = the type's own team only
// (its ping role + any extra support roles). This isolates each type — e.g. only
// @Community Ownership sees Ownership tickets. Falls back to the global support
// roles only when a type defines neither, so generic tickets still have viewers.
